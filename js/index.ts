...
const removeSelected = <HTMLButtonElement>document.getElementById("remove-selected-button")!;
let checks = new Map<string, HTMLTableRowElement>();

const noteContainer = document.getElementById("notes-table")!;

function reloadContainer(notes: Note[]) {
    while (noteContainer.children.length > 0)
        noteContainer.children[0].remove();
    if (notes.length == 0) {
        noteContainer.append(createElement("tr", {},
            createElement("td", { colSpan: 5 },
                createElement("p", { style: { fontSize: "36px", textAlign: "center" }},
                    createElement("span", { textContent: "Adicione uma " }),
                    createElement("a", { href: "./note.html", textContent: "nota" }),
        ...
        noteContainer.append(createElement("tr", {},
            createElement("th", { textContent: "Título" }),
            createElement("th", { textContent: "Status" }),
            createElement("th", { textContent: "Prioridade" }),
            createElement("th", { textContent: "Data de expiração" })
        ));
        for (let note of notes) {
            let priorityColor: string, color: string;
            if (note.isExpired())
            ...
            const tr = createElement("tr", {
                ...
                createElement("td", { style: { backgroundColor: color } },
                    createElement("a", { href: `./note.html?id=${note.id}`, textContent: note.title })
                ),
                createElement("td", { style: { backgroundColor: color } },
                    createElement("p", {
                        style: {
                ...
                createElement("td", { style: { backgroundColor: priorityColor }, textContent: priorityToPtBr(note.priority) }),
                createElement("td", { style: { backgroundColor: color }, textContent: new Date(note.expirationDate).toLocaleString() })
            );
