...
const title = <HTMLInputElement>document.getElementById("note-title")!;
const content = <HTMLInputElement>document.getElementById("note-content")!;
const expirationDate = <HTMLInputElement>document.getElementById("expiration-date")!;
    ...
    if (isNewNote = found === undefined)
        ...
        if (note.isExpired()) {
            title.disabled = content.disabled = expirationDate.disabled =
                (<HTMLButtonElement>document.querySelector("input[type=\"submit\"]")).disabled = true;
...
document.getElementById("note-form")!.onsubmit = event => {
    ...
    let rawExpirationDate = new Date(parseInt(results[3]), parseInt(results[2]) - 1, parseInt(results[1]),
                                     0, 0, 0, 0).valueOf();
    ...
    note.priority = <Priority>(<HTMLInputElement>document.querySelector("input[name=\"priority-radio\"]:checked")).value;
